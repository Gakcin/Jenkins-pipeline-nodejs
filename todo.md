# Jenkins Pipeline to Deploy Docker Images on ECR to ECS cluster
...
- Part 4 - Cleaning up the Image Repository on AWS ECR and ECS cluste
# Prepare infrastructure
-   create ecr-2 folder your local system
-   move ecr-ecs-ec2-jenkins.tf to ecr-2 folder
-   provising github repo using Terraform
    - get your github token for this purpose
    - paste token ecr-ecs-ec2-jenkins.tf locals github-token = "xxxxxxxxxxxxxxxxxxxxxx"
    - update  ecr-ecs-ec2-jenkins.tf locals github-username = "serdarcw" 
    - update  ecr-ecs-ec2-jenkins.tf locals github-email = "aaa@aaa.com"
    - update  ecr-ecs-ec2-jenkins.tf locals key_pair="xxxxx"
    - update  ecr-ecs-ec2-jenkins.tf locals pem_key_address = "~/Downloads/xxxxx.pem"

# Terraform apply
-   terraform init
-   terraform plan
-   terraform init ecr-ecs-ec2-jenkins.tf -auto-approve 
-   provider "aws"
-   provider "github" 
...
-   resource "aws_instance" "jenkins-server   user_data 
    - update os
...
# Configue Jenkins server spinned-up aws

-   open browser jenkins server public ip and 8080 port (http://122.122.111.0:8080)
-   past password joining jenkins with ssh put this command "sudo cat /var/lib/jenkins/secrets/initialAdminPassword" and paste it output into browser
-   install default plug-ins
-   change jenkins user admin password 1234
...
pipeline {
    ...
    stages {
        stage("Run app on Docker"){
            agent{
                docker{
                    image 'node:12-alpine' /* using docker image as agent > Pull docker image from docker hub > run this image and create docker container oven jenkins server */
                }
            }
            steps{
                withEnv(["HOME=${env.WORKSPACE}"]) { /* run sh 'yarn install --production' on docker container located Jenkins server > ckeck http://44.205.6.189:8080/env-vars.html/ on jenkins server*/
                    sh 'yarn install --production' /* downlad packege.json expect dev section*/
                }   
            }
        }
    }
...
```
### Make change to trigger Jenkins
...
```bash
aws ecs create-service --cluster to-do-app --service-name to-do-app-service --task-definition to-do-app --desired-count 1 --launch-type "FARGATE" --network-configuration "awsvpcConfiguration={subnets=[subnet-079c7b24d4bbd9a71],securityGroups=[sg-00721fa2465d411a8],assignPublicIp=ENABLED}" # change the subnets and security group and make sure that the 3000 port is open. 
```
...
git push
```
- Don't forget vdefaut vpc > default security group tcp 3000 any ipv4
- Check the <ecs task Public IP:3000> page.

# Cleaning up the Image Repository on AWS ECR and ECS cluster
